...
export async function addProducts(cartId, products) {
  try {
    const cart = await cartModel.findByIdAndUpdate(cartId, { $addToSet: { products: [] } }, { new: true });
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      const productPrice = await productModel.findOne({ _id: product.product }).select("price");
      ...
      const existingProductIndex = cart.products.findIndex(p => p.product._id.equals(product.product));
